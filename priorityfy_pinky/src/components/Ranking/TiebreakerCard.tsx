...
export function TiebreakerCard() {
  const { currentComparison, handleChoice } = useBubbleSortComparison();
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {[currentComparison.itemA, currentComparison.itemB].map(
                (item) => (
                  <Card
                    key={item.id}
                    className="cursor-pointer hover:shadow-lg transition-all border-2 hover:border-amber-300 dark:hover:border-amber-600 hover:scale-105"
